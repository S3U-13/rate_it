...
@section('content')
    @php
        ...
        $date03 = \Carbon\Carbon::parse($further->first()->further_date ?? null);
        $date04 = \Carbon\Carbon::parse($witness_signature->first()->witness_date ?? null);
    ...
    <div class="w-full max-w-screen-lg h-full border border-gray-300 mx-auto rounded-lg overflow-hidden shadow-2xl">
        <form action="{{ route('page.result.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="pl-[4rem] pr-[3rem]">
                            ...
                            <tr class="border-t">
                                ...
                                <td class="border-r pl-2 text-center bg-gray-200">
                                    ...
                                    <input type="hidden" name="total_points"
                                        value="{{ $total_point ?? 'ยังไม่มีข้อมูล' }}">
                ...
                <div
                    ...
                    <h1 class="pl-2 text-gray-900 font-bold"><span class="text-gray-600">อยู่ในเกณฑ์ :</span>
                        {{ $summarize_01->first()->criterion->criterion_name ?? 'ยังไม่มีข้อมูล' }}</h1>
                </div>
            ...
            <div class="pl-[4rem] pr-[3rem]">
                ...
                <div class="border-r border-l border-b mt-2 rounded-lg overflow-hidden shadow-md">
                    <h1 class="font-bold text-white bg-[#138f3c] pt-2 pb-2 pl-2">ผู้รับประเมิน</h1>
                    <div class="p-2">
                        <h1 class="pl-2">ได้รับทราบผลการประเมินเเละเเผนพัฒนา</h1>

                        <div class="flex justify-end pr-12 pb-4">
                            <div>
                                <div class="flex gap-2">
                                    <div x-data="{
                                        ...
                                        <input type="hidden" name="personal_signature" x-model="signatureUrl">
                                ...
                                <label>วันที่</label>
                                <input type="date" name="personal_date" class="rounded pl-4 p-1 mb-2" required>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="border-r border-l border-b mt-2 rounded-lg overflow-hidden shadow-md">
                    ...
                    <div class="pt-2 pb-2">
                        ...
                        @foreach ($assessmentNums as $num)
                            <span
                                class="pl-3">{{ $assessment_acknowledgement_choice[$num] ?? 'ยังไม่มีการประเมิน' }}</span><br>
                        @endforeach
