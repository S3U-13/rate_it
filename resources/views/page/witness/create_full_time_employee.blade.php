...
@section('title')
    เเบบสรุปการประเมินการปฏิบัติงานของลูกจ้างประจำ(พยาน)
@endsection

@section('content')
    @php
        $date01 = \Carbon\Carbon::parse($personal_signature->first()->personal_date ?? null);
        $date02 = \Carbon\Carbon::parse($summarize_03->first()->evaluator_date ?? null);
    ...
    <div class="w-full max-w-screen-lg h-full border border-gray-300 mx-auto rounded-lg overflow-hidden shadow-2xl">
        <form action="{{ route('page.witness.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="pl-[3rem] pr-[2rem]">
                <div class="px-4 md:px-16 lg:px-24 xl:px-32">
                    <h1 class="font-bold mt-6 text-xl text-left md:text-left">
                        เเบบสรุปการประเมินการปฏิบัติงานของลูกจ้างประจำ(พยาน)</h1>
                                ...
                                <div class="flex justify-end pr-7">
                                    <div>
                                        <div class="flex gap-2">
                                            ...
                                            <div x-data="{
                                                ...
                                                <div class="flex gap-2 items-center mt-2">
                                                    ...
                                                    <span class="text-gray-900">พยาน</span>
                                                ...
                                                <input type="hidden" name="witness_signature" x-model="signatureUrl">
                                        ...
                                        <div class="">
                                            ...
                                            <input type="date" name="witness_date" class="rounded pl-4 p-1 mb-2"
                                                required>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="flex flex-col md:flex-row justify-between px-4 pb-4">
                            ...
                            @if ($above->first() && !empty($above->first()->above_comment_num === 1))
                                <div class="md:w-2/3 mt-2">
                                    <span>{{ $above->first()->comments->comment_name ?? '' }}</span><br>
                                </div>
                            @elseif($above->first() && !empty($above->first()->above_comment_num === 2))
                                <div class="md:w-2/3 mt-2">
                                    <span>{{ $above->first()->comments->comment_name ?? 'ยังไม่มีข้อมูล' }}</span><br>
                                    <span>ดังนี้</span>
                                    <div class="border rounded p-2 mb-2 md:w-[25rem] ">
                                        <h1>(๑) การให้คะแนนในการประเมิน</h1>
                                        {{ $above->first()->above_comment_detail_1 ?? 'ยังไม่มีข้อมูล' }}
                                        <h1>(๒) การพัฒนาผู้รับการประเมิน</h1>
                                        {{ $above->first()->above_comment_detail_2 ?? 'ยังไม่มีข้อมูล' }}
                                        <h1>(๓) การเลื่อนขั้นค่าจ้าง</h1>
                                        {{ $above->first()->above_comment_detail_3 ?? 'ยังไม่มีข้อมูล' }}
                                    </div>
                                </div>
                            @else
                            ...
                            <div class=" md:w-1/3 mt-4 ml-[8rem]">
                                ...
                                <h1 class="pl-12">({{ $above->first()->above->name ?? 'ยังไม่มีข้อมูล' }})
                                </h1>
                    ...
                    <div class="border mt-2 rounded-lg overflow-hidden">
                        ...
                        <div class="flex flex-col md:flex-row justify-between px-4 pb-4">
                            ...
                            @if ($further->first() && !empty($further->first()->further_comment_num === 1))
                                <div class="md:w-2/3 mt-2">
                                    <span>{{ $further->first()->comments->comment_name ?? '' }}</span><br>
                                </div>
                            @elseif ($further->first() && !empty($further->first()->further_comment_num === 2))
                                <div class="md:w-2/3 mt-2">
                                    ...
                                    <div class="border rounded p-2 mb-2 md:w-[25rem]">
                                        <h1>(๑) การให้คะแนนในการประเมิน</h1>
                                        {{ $further->first()->further_comment_detail_1 ?? 'ยังไม่มีข้อมูล' }}
                                        <h1>(๒) การพัฒนาผู้รับการประเมิน</h1>
                                        {{ $further->first()->further_comment_detail_2 ?? 'ยังไม่มีข้อมูล' }}
                                        <h1>(๓) การเลื่อนขั้นค่าจ้าง</h1>
                                        {{ $further->first()->further_comment_detail_3 ?? 'ยังไม่มีข้อมูล' }}
                                    </div>
                                </div>
                            @else
                                <span>ยังไม่มีข้อมูล</span>
                            ...
                            <div class=" md:w-1/3 mt-4 ml-[8rem]">
                                ...
                                <h1 class="pl-12">
                                    ({{ $further->first()->further->name ?? 'ยังไม่มีข้อมูล' }})
                                </h1>
                                <div class="flex gap-2 justify-none md:justify-start">
                                    <h1>ตำแหน่ง</h1>
                        ...
                        <div class="text-gray-900 border-gray-400 border rounded-lg overflow-hidden shadow-md">
                            ...
                            <div class="p-2 text-sm">
                                ...
                                <h1 class="pl-2"><span class="pr-6">ส่วนที่ ๒ </span><ins class="pr-2">
                                    ...
                                    องค์ประกอบด้านพฟติกรรมการปฏิบัติราชการเเละน้ำหนักของทั้งสององค์ประกอบ
                                    ในเเบบสรุปส่วนที่
                                    ๒
                                    นี้
                                ...
                                <h1 class="pl-2"><span class="pr-6">ส่วนที่ ๓</span><ins class="pr-2">
                                    ...
                                    ผู้ประเมินเเละผู้รับการประเมินร่วมกันจัดทำเเบบเเผนพัฒนาผลการปฏิบัติราชการ
                                </h1>
                                ...
                                <h1 class="pl-2"><span class="pr-6">ส่วนที่ ๓</span><ins class="pr-2">
                                    ...
                                    ความเห็นของผู้บังคับบัญชาเหนือขึ้นไปกลั่นกรองผลการประเมิน
                                    เเผนพัฒนาผลการปฏิบัติราชการ
                                    เเละให้ความเห็น คำว่า "ผู้บังคับบัญชาเหนือขึ้นไป" สำหรับผู้ประเมินตามข้อ ๒
                                    (๙)
                                    หมายถึง
