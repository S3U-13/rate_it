        ...
        <div class="bg-white shadow-md rounded-lg overflow-hidden">
            <div class="bg-[#138f3c] text-white text-center py-3">
                @if ($evaluationRound == 1)
                    <h5 class="text-lg font-semibold">📋 ลงนามพยานรอบที่ ๑ ประจำปี {{ $thaiNumbers }}</h5>
                @elseif($evaluationRound == 2)
                    <h5 class="text-lg font-semibold">📋 ลงนามพยานรอบที่ ๒ ประจำปี {{ $thaiNumbers }}</h5>
                @endif
            </div>
                            ...
                            @elseif($evaluationRound == 2)
                                <tr>
                                    <td colspan="5" class="px-4 py-6 border">
                                        <div class="bg-green-100 text-green-700 p-4 rounded-md">
                                            <i class="bi bi-emoji-smile-fill text-3xl"></i><br>
                                            ขอบคุณสำหรับการทำแบบประเมินใน รอบที่ ๒ ประจำปี {{ $thaiNumbers }} 🎉<br>
                                            คุณประเมินครบทุกคนแล้วในรอบนี้! <span
                                                class="text-red-500">หรือยังไม่มีผู้ลงนาม</span>
                                        </div>
                                    </td>
                        ...
                        @else
                            @foreach ($personals as $item)
                                <tr class="border border-[#006622] hover:bg-gray-100">
                                    ...
                                    <td class="px-4 py-2 text-left border border-[#006622]">{{ $item->JobPosition->job_position_name }}
                                    </td>
                                    <td class="px-4 py-2 border border-[#006622]">
                                        <!-- ปุ่มให้คะแนน -->
                                        @if (!$item->witnessSignature->where('round', $evaluationRound)->isEmpty())
                                            <!-- ถ้ามีพยานลงชื่อในรอบนี้ จะไม่แสดงปุ่ม -->
                                            <span>มีพยานลงชื่อแล้ว</span>
                                        @else
                                            <!-- ถ้าไม่มีพยานลงชื่อในรอบนี้ จะมีปุ่มให้ลงชื่อ -->
                                            @switch($item->job_type_num)
                                                @case(1)
                                                    <a href="{{ route('page.witness.create', $item->id) }}"
                                                        class="bg-[#138f3c] text-white px-3 py-1 rounded-md text-sm hover:bg-green-700">
                                                        <i class="bi bi-pencil-square"></i> ลงชื่อพยาน
                                                    </a>
                                                @break

                                                @case(2)
                                                    <a href="{{ route('page.witness.create_full_time_employee', $item->id) }}"
                                                        class="bg-[#138f3c] text-white px-3 py-1 rounded-md text-sm hover:bg-green-700">
                                                        <i class="bi bi-pencil-square"></i> ลงชื่อพยาน
                                                    </a>
                                                @break
                                                ...
                                                @case(3)
                                                    <a href="{{ route('page.witness.create_general_government_employee', $item->id) }}"
                                                        class="bg-[#138f3c] text-white px-3 py-1 rounded-md text-sm hover:bg-green-700">
                                                        <i class="bi bi-pencil-square"></i> ลงชื่อพยาน
                                                    </a>
                                                @break
                                                ...
                                                @case(4)
                                                    <a href="{{ route('page.witness.create_ministry_of_public_health_employee', $item->id) }}"
                                                        class="bg-[#138f3c] text-white px-3 py-1 rounded-md text-sm hover:bg-green-700">
                                                        <i class="bi bi-pencil-square"></i> ลงชื่อพยาน
                                                    </a>
                                                @break
                                                ...
                                                @case(5)
                                                    <a href="{{ route('page.witness.create_temporary_employee', $item->id) }}"
                                                        class="bg-[#138f3c] text-white px-3 py-1 rounded-md text-sm hover:bg-green-700">
                                                        <i class="bi bi-pencil-square"></i> ลงชื่อพยาน
                                                    </a>
                                                @break
