    ...
    <div class="w-full max-w-screen-lg h-full border border-gray-300 mx-auto rounded-lg overflow-hidden shadow-2xl">
        <form action="{{ route('page.capacity_rate_it.further.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <h1 class="pt-[2rem] pb-[2rem] text-center bg-[#138f3c] text-white text-xl">📋
                หน้าเพิ่มเเบบสรุปการประเมินการปฏิบัติงานของพนักงานราชการทั่วไป
            ...
            <div class="pl-[3rem] pr-[2rem]">
                <div class="px-4 md:px-16 lg:px-24 xl:px-32">
                    <h1 class="font-bold mt-6 text-xl text-left md:text-left">
                        เเบบสรุปการประเมินการปฏิบัติงานของพนักงานราชการทั่วไป
                    </h1>
                    ...
                    <div class="rounded-lg overflow-hidden shadow-md border border-gray-400 mt-2">
                        ...
                        <div class="bg-gray-100 p-4">
                            ...
                            <div class="flex flex-col md:flex-row md:justify-between">
                                <div class="mb-2">
                                    <h1 class="text-gray-600 inline">วันเริ่มสัญญา:</h1>
                                    <span class="text-gray-900 font-bold ml-2">
                                        {{ toThaiNumber(\Carbon\Carbon::parse($personal->contract_start_date)->locale('th')->addYears(543)->isoFormat('D MMMM YYYY')) }}
                                    </span>
                                </div>
                                <div class="mb-2">
                                    <h1 class="text-gray-600 inline">วันสิ้นสุดสัญญาจ้าง:</h1>
                                    <span class="text-gray-900 font-bold ml-2 pr-[8.4rem]">
                                        {{ toThaiNumber(\Carbon\Carbon::parse($personal->end_date_of_employment_contract)->locale('th')->addYears(543)->isoFormat('D MMMM YYYY')) }}
                                    </span>
                                </div>
                            </div>
            ...
            <div class="pl-[4rem] pr-[3rem]">
                                    ...
                                    <td class="border-r pl-2 text-center p-1">
                                        @php
                                            ...
                                            if ($evaluationRound == 2) {
                                                $total_point_round_1 = $personal_round_1->first()->total_points ?? null;
                                                $average_total = ($total_point + $total_point_round_1) / 2;
                                            }
                                        @endphp
                ...
                @if ($evaluationRound == 2)
                    <h1 class="mt-2">ผลการประเมินเฉลี่ย
                        2 รอบการประเมิน</h1>
                    <div class="border p-2 rounded">

                        <div class="flex gap-2 mt-2">
                            <h1 class="border-b w-20 text-center">{{ $total_point_round_1 }} + {{ $total_point }}
                            </h1>
                            @if ($average_total > 90)
                                <h1> = {{ $average_total }} อยู่ในเกณฑ์ ดีเด่น</h1>
                            @elseif ($average_total > 80)
                                <h1> = {{ $average_total }} อยู่ในเกณฑ์ ดีมาก</h1>
                            @elseif ($average_total > 70)
                                <h1> = {{ $average_total }} อยู่ในเกณฑ์ ดี</h1>
                            @elseif ($average_total > 60)
                                <h1> = {{ $average_total }} อยู่ในเกณฑ์ พอใช้</h1>
                            @else
                                <h1> = {{ $average_total }} อยู่ในเกณฑ์ ต้องปรับปรุง</h1>
                            @endif
                        </div>
                        <h1 class="text-center w-18">2</h1>
                    </div>
                @endif
                        ...
                        <div class="border mt-2 rounded-lg overflow-hidden">
                            ...
                            <div class="flex flex-col md:flex-row justify-between px-4 pb-4">
                                <div id="textarea-container02" style="display: none;" class="md:w-2/3 mt-2">
                                    <span>ดังนี้</span><br>
                                    <textarea name="further_comment_detail"
                                        class="border border-gray-400 p-2 mb-2 w-full max-w-[25rem] h-[6rem] rounded-lg" id="comment-textarea02"></textarea>
                                </div>
                                <script>
                                    // เมื่อเลือก radio
