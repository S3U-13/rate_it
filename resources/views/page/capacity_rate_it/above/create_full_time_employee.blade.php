...
@section('title')
    เพิ่มความคิดเห็นของผู้บังคับบัญชาเหนือขึ้นไป
@endsection
    ...
    <div class="w-full max-w-screen-lg h-full border border-gray-300 mx-auto rounded-lg overflow-hidden shadow-2xl">
        <form action="{{ route('page.capacity_rate_it.above.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <h1 class="pt-[2rem] pb-[2rem] text-center bg-[#138f3c] text-white text-xl">📋
                หน้าเพิ่มเเบบสรุปการประเมินการปฏิบัติงานของ ลูกจ้างประจำ
                (ผู้บังคับบัญชาเหนือขึ้นไป) </h1>
                    ...
                    <div class="rounded-lg overflow-hidden shadow-md border border-gray-400 mt-2">
                        ...
                        <div class="bg-gray-100 p-4">
                            ...
                            <div class="mb-2">
                                ...
                                <span
                                    class="text-gray-900 font-bold ml-2">{{ $summarize_01->first()->evaluator->name ?? 'ยังไม่มีข้อมูล' }}</span>
                            </div>

                            <div class="mb-2">
                                <h1 class="text-gray-600 inline">ตำเเหน่ง:</h1>
                                <span
                                    class="text-gray-900 font-bold ml-2">{{ $summarize_01->first()->evaluator->JobPosition->job_position_name ?? 'ยังไม่มีข้อมูล' }}
            ...
            <div>
                <div class="pl-[3rem] pr-[2rem]">
                    ...
                    <div class="px-4 md:px-16">
                        <div class="border mt-2 rounded-lg overflow-hidden">
                            <h1 class="font-bold bg-[#138f3c] py-4 px-4 text-white mb-2">ผู้บังคับบัญชาเหนือขึ้นไป</h1>
                            @foreach ($comment_choice as $comment)
                                <div class="pl-4 mt-2">
                                    <input type="radio" name="above_comment_num" value="{{ $comment->id }}"
                                        id="comment_{{ $comment->id }}">
                                    <span class="pl-2">{{ $comment->comment_name }}</span>
                                </div>
                            @endforeach
                            <div class="flex flex-col md:flex-row justify-between px-4 pb-4">
                                <div id="textarea-container" style="display: none;" class="md:w-2/3 mt-2">
                                    <span>ดังนี้</span><br>
                                    <div>
                                        <label for="above_comment_detail_1">(1) การให้คะแนนในการประเมิน</label>
                                        <input name="above_comment_detail_1"
                                            class="border border-gray-400 p-2 mb-2 w-full max-w-[25rem] rounded-lg"
                                            id="comment-textarea02"></input>
                                    </div>
                                    <div> <label for="above_comment_detail_2">(2) การพัฒนาผู้รับการประเมิน</label>
                                        <input name="above_comment_detail_2"
                                            class="border border-gray-400 p-2 mb-2 w-full max-w-[25rem] rounded-lg"
                                            id="comment-textarea02"></input>
                                    </div>
                                    <div> <label for="above_comment_detail_3">(3) การเลื่อนขั้นค่าจ้าง</label><br>
                                        <input name="above_comment_detail_3"
                                            class="border border-gray-400 p-2 mb-2 w-full max-w-[25rem] rounded-lg"
                                            id="comment-textarea02"></input>
                                    </div>
                                </div>
                                ...
                                <div class="md:w-1/3 mt-4">
                                    ...
                                    <label>วันที่</label>
                                    <input type="date" name="above_date" class=" rounded pl-2 p-1" required>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="pl-[4rem] pr-[3rem] mt-6 mb-6 ">
