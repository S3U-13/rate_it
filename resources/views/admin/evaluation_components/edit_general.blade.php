...
@section('title')
    แก้ไของค์ประกอบการประเมินทั่วไป
@endsection

@section('content')
    <div class="p-4 sm:ml-64">
        <div class="mt-14 max-w-3xl mx-auto">
            <h1 class="pt-6 text-2xl font-semibold text-gray-800">แก้ไของค์ประกอบทั่วไป</h1>
            <div class="bg-white rounded-lg shadow-xl mt-6 border border-gray-200 p-6">
                <form action="{{ route('admin.evaluation_components.update_general', $EvaluationComponentGeneral->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="space-y-6">
                        <!-- Field คำถาม -->
                        <div>
                            ...
                            <input type="text" name="component_name" id="component_name"
                                value="{{ $EvaluationComponentGeneral->component_name }}"
                            ...
                            <div class="mt-2 flex items-center space-x-6">
                                <label class="inline-flex items-center">
                                    <input type="radio" name="component_status" value="on"
                                        {{ $EvaluationComponentGeneral->component_status === 'on' ? 'checked' : '' }}
                                        class="form-radio text-indigo-600">
                                    <span class="ml-2">ทำงาน</span>
                                </label>
                                <label class="inline-flex items-center">
                                    <input type="radio" name="component_status" value="off"
                                        {{ $EvaluationComponentGeneral->component_status === 'off' ? 'checked' : '' }}
                                        class="form-radio text-indigo-600">
                        ...
                        <div>
                            ...
                            <input type="text" name="weight_score" id="weight_score"
                                value="{{ $EvaluationComponentGeneral->weight_score }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
