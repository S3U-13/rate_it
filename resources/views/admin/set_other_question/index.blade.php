    ...
    <div class="p-2 sm:ml-64">
        <div class="pt-2 rounded-lg shadow-xl mt-14 pb-4">
            ...
            @elseif(session('error'))
                <div
                    class="bg-red-100 text-red-700 text-center p-4 rounded-md shadow-sm flex justify-between items-center">
                  ...
                  <div class="overflow-x-auto">
                    ...
                    <table class="min-w-full border border-gray-300">
                      <thead class="bg-[#138f3c] text-white text-center">
                        <tr>
                          <th class="px-4 py-2 border w-1/6">ลำดับ</th>
                          <th class="px-4 py-2 border w-2/6">คำถาม</th>
                          <th class="px-4 py-2 border w-1/6">สถานะ</th>
                          <th class="px-4 py-2 border w-1/6">น้ำหนัก (ตัวคูณ)</th>
                          <th class="px-4 py-2 border w-3/6">การดำเนินการ</th>
                        </tr>
                      </thead>
                      <tbody class="text-center">
                        @foreach ($other_questions as $item)
                          <tr class="border hover:bg-gray-100">
                            <td class="px-4 py-2 border">{{ $loop->iteration }}</td>
                            <td class="px-4 py-2 text-left border">{{ $item->other_question_name }}</td>
                            <td class="px-4 py-2 text-left border">{{ $item->other_question_status }}</td>
                            <td class="px-4 py-2 text-left border">
                              {{ $item->other_question_multiply ?? 'ยังไม่ได้ตั้งค่า' }}
                            </td>
                            <td class="px-4 py-2 border">
                              <div class="flex justify-center space-x-2">
                                <a href="{{ route('admin.set_other_question.edit', $item->id) }}"
                     ...
                     <div class="flex justify-between items-center mt-4 p-2">
                            <p class="text-[#006622] ml-[1rem]">
                                จำนวนข้อมูลในหน้าปัจจุบัน: {{ $other_questions->count() }} |
                                แสดงข้อมูลจาก: {{ $other_questions->firstItem() }} ถึง {{ $other_questions->lastItem() }} |
                                จำนวนข้อมูลทั้งหมด: {{ $other_questions->total() }}
                            </p>

                            <div class="flex gap-2 mr-[1rem]">
                                <!-- ปุ่มย้อนกลับ -->
                                @if ($other_questions->previousPageUrl())
                                    <a href="{{ $other_questions->appends(['entries' => $entries])->previousPageUrl() }}"
                                ...
                                <!-- ปุ่มถัดไป -->
                                @if ($other_questions->nextPageUrl())
                                    <a href="{{ $other_questions->appends(['entries' => $entries])->nextPageUrl() }}"
