    ...
    <div class="p-2 sm:ml-64">
        <div class="flex items-center justify-center min-h-screen bg-gray-100 mt-15">
            <div class="w-full max-w-lg p-6 bg-white rounded-lg shadow-md">
                <h2 class="text-2xl font-semibold text-center text-gray-800 mb-4">เเก้ไขผู้ใช้งาน</h2>

                <form action="{{ route('admin.personal.update_user', $user->id) }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PUT')
                    <div>
                    ...
                    <div>
                        ...
                        <select name="group_num" id="group_num"
                            ...
                            @foreach ($group as $item)
                                <option value="{{ $item->id }}" {{ $user->group_num == $item->id ? 'selected' : '' }}>
                        ...
                        <select name="work_affiliation_num" id="work_affiliation_num"
                            ...
                            @foreach ($work_affiliation as $item)
                                <option value="{{ $item->id }}" data-group="{{ $item->group_num }}"
                                    {{ $user->work_affiliation_num == $item->id ? 'selected' : '' }}>
                                    {{ $item->work_affiliation_name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="job_position_num" id="job_position_num"
                            ...
                            @foreach ($job_position as $item)
                                <option
                                    value="{{ $item->id }}" {{ $user->job_position_num == $item->id ? 'selected' : '' }}>
                                    {{ $item->job_position_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="job_type_num" id="job_type_num"
                            ...
                            @foreach ($job_type as $item)
                                <option
                                    value="{{ $item->id }}" {{ $user->job_type_num == $item->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="tier_num" id="tier_num"
                            ...
                            @foreach ($tier as $item)
                                <option value="{{ $item->id }}" {{ $user->tier_num == $item->id ? 'selected' : '' }}>
                                    {{ $item->tier_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input name="end_date_of_employment_contract" id="end_date_of_employment_contract" type="date"
                            value="{{ $user->end_date_of_employment_contract }}"
                            class="w-full mt-1 p-2 border rounded-lg focus:ring focus:ring-gray-300 text-gray-700">
                        </input>
                    </div>
                    <div>
                        ...
                        <input name="wages" id="wages" type="number" value="{{ $user->wages }}"
                            class="w-full mt-1 p-2 border rounded-lg focus:ring focus:ring-gray-300 text-gray-700">
                        </input>
                    </div>
                    <div>
                    ...
                    <button type="submit"
                        class="w-full p-2 text-white bg-[#138f3c] rounded-lg hover:bg-green-700">ยืนยันการเเก้ไข</button>
                </form>
            </div>
        </div>
