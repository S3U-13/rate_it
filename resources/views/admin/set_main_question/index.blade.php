                    ...
                    <div class="overflow-x-auto">
                        ...
                        <table class="min-w-full border border-gray-300">
                            ...
                            <tbody class="text-center">
                                @foreach ($main_questions as $item)
                                    <tr class="border hover:bg-gray-100">
                                        <td class="px-4 py-2 border">{{ $loop->iteration }}</td>
                                        <td class="px-4 py-2 text-left border">{{ $item->main_question_name }}</td>
                                        <td class="px-4 py-2 text-left border">{{ $item->main_question_status }}</td>
                                        <td class="px-4 py-2 text-left border">
                                            {{ $item->main_question_multiply ?? 'ยังไม่ได้ตั้งค่า' }}
                                        </td>
                                        <td class="px-4 py-2 border">
                                            <div class="flex justify-center space-x-2">
                                                <a href="{{ route('admin.set_main_question.edit', $item->id) }}"
                         ...
                         <div class="flex justify-between items-center mt-4 p-2">
                            <p class="text-[#006622] ml-[1rem]">
                                จำนวนข้อมูลในหน้าปัจจุบัน: {{ $main_questions->count() }} |
                                แสดงข้อมูลจาก: {{ $main_questions->firstItem() }} ถึง {{ $main_questions->lastItem() }} |
                                จำนวนข้อมูลทั้งหมด: {{ $main_questions->total() }}
                            </p>

                            <div class="flex gap-2 mr-[1rem]">
                                <!-- ปุ่มย้อนกลับ -->
                                @if ($main_questions->previousPageUrl())
                                    <a href="{{ $main_questions->appends(['entries' => $entries])->previousPageUrl() }}"
                                ...
                                <!-- ปุ่มถัดไป -->
                                @if ($main_questions->nextPageUrl())
                                    <a href="{{ $main_questions->appends(['entries' => $entries])->nextPageUrl() }}"
