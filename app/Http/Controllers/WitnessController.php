...
class WitnessController extends Controller
{
    ...
    public function index()
    {
        ...
        $personals = User::where('group_num', $user->group_num)
            ->where('id', '!=', $user->id) // ไม่รวมผู้ใช้ตัวเอง
            ->whereNotIn('tier_num', [4, 11, 12]) // ไม่รวม tier ที่ไม่ต้องการ
            ->where('role', 'user') // แค่ผู้ใช้ที่มี role = 'user'
            ->whereHas('personalSignature', function ($query) use ($evaluationRound) {
            ...
            ->whereDoesntHave('witnessSignature', function ($query) use ($evaluationRound) {
                // ตรวจสอบว่าไม่ได้ลงชื่อเป็นพยานในรอบนี้
                $query->where('round', $evaluationRound);
            })
            ->get()
            ->filter(function ($personal) use ($evaluationRound) {
                // ตรวจสอบว่า personal มี round ที่ตรงกับ evaluationRound
                return $personal->round = $evaluationRound;
            });


        $hasCompletedEvaluation = $personals->isEmpty();

        return view('page.witness.index', compact('personals', 'hasCompletedEvaluation', 'evaluationRound', 'now',));
    }
    public function create($id)
    {
        ...
        // ดึงข้อมูลเฉพาะที่ round ตรงกับ $evaluationRound
        ...
        $summarize_03 = SummarizePart03::where('personal_num', $personal->id)
            ->where('round', $evaluationRound)->get();
        $above = AboveComment::where('personal_num', $personal->id)
            ->where('round', $evaluationRound)->get();
        $further = FurtherComment::where('personal_num', $personal->id)
            ->where('round', $evaluationRound)->get();
        $personal_signature = PersonalSignature::where('personal_num', $personal->id)
            ->where('round', $evaluationRound)->get();
        ...
        return view('page.witness.create', compact(
            'personal',
            'summarize_01',
            'summarize_02',
            'summarize_03',
    ...
    public function create_full_time($id)
    {
        ...
        return view('page.witness.create_full_time_employee', compact(
            'personal',
            'summarize_01',
            'summarize_02',
            'summarize_03',
    ...
    public function create_general_government($id)
    {
        ...
        return view('page.witness.create_general_government_employee', compact(
            'personal',
            'summarize_01',
            'summarize_02',
            'summarize_03',
    ...
    public function create_ministry_of_public_health($id)
    {
        ...
        return view('page.witness.create_ministry_of_public_health_employee', compact(
            'personal',
            'summarize_01',
            'summarize_02',
            'summarize_03',
    ...
    public function create_temporary($id)
    {
        ...
        return view('page.witness.create_temporary_employee', compact(
            'personal',
            'summarize_01',
            'summarize_02',
            'summarize_03',
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('page.witness.index')->with('success', 'เพิ่มเเบบสรุปผลการปฏิบัติราชการสำเร็จ!');
        } catch (\Throwable $th) {
            //throw $th;
            Log::error('Error occurred while storing data: ' . $th->getMessage());

            return redirect()->route('page.witness.index')->with('error', 'เกิดข้อผิดพลาด กรุณาลองใหม่');
        }
    }
